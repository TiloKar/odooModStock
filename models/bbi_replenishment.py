...
FROM (SELECT
    ...
    FROM
        GENERATE_SERIES((now() at time zone 'utc')::date - interval '48month',
        (now() at time zone 'utc')::date + interval '48 month', '1 day'::interval) date,
        stock_quant q
    ...
    SELECT
        ...
        GENERATE_SERIES(
        CASE
            WHEN m.state = 'done' THEN (now() at time zone 'utc')::date - interval '48month'
            ELSE m.date::date
        END,
        CASE
            WHEN m.state != 'done' THEN (now() at time zone 'utc')::date + interval '48 month'
...
class StockWarehouseOrderpoint(models.Model):
    ...
    def _get_orderpoint_action(self):
        ...
        # EDIT: Hanning Liu months von 3 auf 48 gesetzt um die Leadtage zu erhöhen
        to_date = add(fields.date.today(), months=48)
